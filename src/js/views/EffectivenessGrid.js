Repository.Local.Methods.initialize({
    delayedAffixFn: new Ext.util.DelayedTask(function(grid, headerEl, $doc) {
        ...
        if (headerEl.getWidth()) {
            headerEl.setY(Math.max(Ext.getBody().getScrollTop(), grid.getBox().y));
        }
    }),
    ...
    getEffectivenessValues: function() {
        ...
        Ext.Array.forEach(db.Indicators, function(rc) {
            ...
            if (rc.Period === pc && rc.YearOfPeriod === yc && rc.Gender === gc && rc.Administration.length === 5) {
                if (!ev[is]) {
                    ev[is] = {
                        register: Repository.Local.Methods.toRegisterCode(rc.Indicator),
                        indicator: rc.Indicator,
                        sequence: Repository.Local.Methods.getIndicatorSequence(rc.Indicator)
                    };
                }
    ...
    paintIndicatorHeat: function(scope) {
        return function(aValue, aMeta, aRecord, aCellY, aCellX, aStore, aView) {
            var t = Repository.Local.Methods.getIndicatorTargets(aRecord.get('indicator')),
            ...
            if (Ext.isEmpty(aValue) || aValue.Measure === null) {
                ...
                if (t.LimitBelow < t.LimitAbove) { // Not the case for reversed indicators.
                ...
                m = Ext.String.format('{0}<br/>{1} observationer.<br/>{2}. Mål {3}.',
                    Repository.Local.Methods.mapManagementCodeToName(aValue.Administration),
                    aValue.Size,
            ...
            return aMeta.tdCls ? ('<img src="' + widget.getLimitColorImage(aMeta.tdCls) + '" style="display: none; width: 100%; height: 100%">') : null;
        };
    },
    paintIndicatorName: function(aValue, aMeta, aRecord) {
        //  aMeta.style = 'white-space: inherit;';
        aMeta.tdCls = 'HeatGridName';
        return '<i>' + Repository.Local.Methods.mapTitleCodeToName(aValue) + '</i><br>' + Repository.Local.Methods.mapIndicatorCodeToName(aValue);
    },
    getIsManagementRegistering: function(gridRecord){
        var store = Ext.StoreManager.lookup('KVIndicatorStore');
        var indicatorRecord = store.findRecord('valueCode', gridRecord.get('indicator'));
        var domainValueID, tmp = '';
        if (indicatorRecord) {
            domainValueID = indicatorRecord.get('DomainValueID');
            Repository.Local.Methods.ajaxCall('/api/metadata/domainvalues/' + domainValueID, function (res, json) {
                var children = json.result && json.result.data && json.result.data.Children;
    ...
    getHospitalsInManagement: function(managementCode) {
        ...
        Ext.Object.each(Repository.Local.domainMaps.hospital, function(key, value) {
    ...
    getEffectivenessColumns: function() {
        var widget = this,
            management = Ext.Object.getKeys(Repository.Local.Methods.mapManagementCodeToName() || {}),
            indicatorName = {
                text: 'Indikator',
                dataIndex: 'indicator',
                flex: 1,
        ...
        Ext.each(management, function(m) {
            arr.push({
                ...
                tooltip: '<b>' + Repository.Local.Methods.mapManagementCodeToName(m) + '</b><br>' + widget.getHospitalsInManagement(m).join('<br>'), //Repository.Local.Methods.mapManagementCodeToName(m),
                dataIndex: 'm' + m,
    ...
    init: function(_m) {
        ...
        //     return;
        // }
        typeof EffectivenessModel === 'undefined' && Ext.define('EffectivenessModel', {
            ...
            items: [{
                ...
                listeners: {
                    select: function(aCombo, aSelection) {
                        ...
                        Ext.data.StoreManager.lookup('EffectivenessStore').loadData(widget.getEffectivenessValues());
                    }
                }
            }, {
                xtype: 'combobox',
        ...
        Ext.create('Ext.grid.Panel', {
            ...
            listeners: {
                cellclick: function(aGrid, aTD, aCellX, aRecord) {
                    Repository.Local.current.indicator = aRecord.get('indicator');
                    if (aCellX === 0) {
                        _m.navigateToPage(1275);
                    } else {
                        var m = aRecord.get(aGrid.getGridColumns()[aCellX].dataIndex);
                        if (m && m.Measure !== null) { // Only show results if there are any.
